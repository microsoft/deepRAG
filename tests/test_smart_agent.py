...
def setup_mock_azure_openai_with_side_effects(
    ...
    mockAzureOpenAI: Mock | Mock = mocker.Mock(target=AzureOpenAI, chat=mocker.Mock(completions=mocker.Mock(create=mocker.Mock())))
...
def test_for_valid_response(mocker: pytest_mock.MockerFixture) -> None:
    ...
    mockAzureOpenAI = setup_mock_azure_openai(mocker=mocker, chat_completion_response=chat_completion_response)
    smart_agent: Smart_Agent = setup(mocker=mocker, mockAzureOpenAI=mockAzureOpenAI)
    smart_agent_response: AgentResponse = smart_agent.run(smart_agent_prompt)
...
def test_for_no_user_input(mocker: pytest_mock.MockerFixture) -> None:
    ...
    mockAzureOpenAI = setup_mock_azure_openai(mocker=mocker, chat_completion_response="Assistant Response")
    smart_agent: Smart_Agent = setup(mocker=mocker, mockAzureOpenAI=mockAzureOpenAI)
...
def test_if_conversation_is_not_none(mocker: pytest_mock.MockerFixture) -> None:
    ...
    conversation_item = {"role": "user", "content": "Initial Conversation Item"}
    mockAzureOpenAI = setup_mock_azure_openai(mocker=mocker, chat_completion_response="Assistant Response")
    smart_agent: Smart_Agent = setup(mocker=mocker, mockAzureOpenAI=mockAzureOpenAI)
    smart_agent_response: AgentResponse = smart_agent.run(user_input="Hello World", conversation=[conversation_item])
...
def test_for_max_run_count(mocker: pytest_mock.MockerFixture) -> None:
    ...
    chat_completion_tools: List[ChatCompletionMessageToolCall] = [
        ChatCompletionMessageToolCall(id="foo", type="function", function=Function(name="test", arguments="{}"))
    ]
    ...
    smart_agent_response: AgentResponse = smart_agent.run(user_input="Hello World")
...
def test_for_tool_calls(mocker: pytest_mock.MockerFixture) -> None:
    ...
    tool_message = {'tool_call_id': tool_id, 'role': 'tool', 'name': tool_name, 'content': []}
    ...
    mockAzureOpenAI = setup_mock_azure_openai_with_side_effects(
        mocker=mocker,
        chat_completion_response="Assistant Response",
        chat_completion_finish_reason="tool_calls",
        chat_completion_tool_calls=chat_completion_tools,
