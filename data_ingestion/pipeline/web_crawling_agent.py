...
def extract_content_from_url(client: AzureOpenAI, engine: str, session: HTMLSession, url, retries=0):  
    if retries > 1:  
    ...
    try:  
        ...
        return extract_content_from_url(client, engine, session, url, retries + 1)  # Retry with incremented retry count  
...
def get_image_description(client: AzureOpenAI, processing_engine: str, image_url, retries=0):  
    max_retries = 2
    try:  
        response = client.chat.completions.create(  
        ...
        if retries < max_retries:  
            ...
            return get_image_description(client, processing_engine, image_url, retries + 1)  
        else:  
            print(f"Max retries reached for {image_url}. Skipping.")  
            return f"[Description for {image_url} not available due to error.]"  
          
def replace_image_urls_with_descriptions(client: AzureOpenAI, processing_engine: str, extracted_content, image_urls):  
    with concurrent.futures.ThreadPoolExecutor() as executor:  
        partial_get_image_description = partial(get_image_description, client, processing_engine)
        image_descriptions = list(executor.map(partial_get_image_description, image_urls))  
...
def process_single_url(client: AzureOpenAI, processing_engine: str, engine:str, session: HTMLSession, url, processed_urls, current_depth, depth_limit, lock):  
    ...
    extracted_content = extract_content_from_url(client, engine, session, url)  
    ...
    updated_content = replace_image_urls_with_descriptions(client, processing_engine, extracted_content, image_urls)  
  
    with lock:  
        processed_urls.add(url)  # Mark URL as processed  
  
    return {"url": url, "title": title, "content": updated_content}  
def process_urls_and_write_to_file(
        client: AzureOpenAI,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:  
        while urls_to_process and processed_entries < max_entries: 
            ...
            futures = {  
                executor.submit(process_single_url, client, processing_engine, engine, session, url, processed_urls, current_depth, depth_limit, lock): (url, current_depth)  
